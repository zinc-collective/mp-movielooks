...
- (void)dealloc {
	[readMovieURL release];
	[readMovieAsset release];
	if(tempWriteMoviePath)
		[tempWriteMoviePath release];
	if(prevWriteMoviePath) //bret movielooks update
		[prevWriteMoviePath release];
	if(avComposeSession)
		[avComposeSession release];
	[avMovieCondition release];
    [super dealloc];
}
...
-(void)waitCompleteThreadWithoutAudio
{
	...
	{
		[self.delegate performSelectorOnMainThread:@selector(errorSamplerMovieEvent) withObject:nil waitUntilDone:NO];
	}
...
//	@try{
        ...
        {
			NSLog(@"Video Rendering last Time(%d,%d)!",lastSampleTimeRange.duration.value,lastSampleTimeRange.duration.timescale);
...
//	}
	...
	avMovieReader = [[AVAssetReader assetReaderWithAsset:readMovieAsset error:&error] retain];
	if(error)
	{	NSLog(@"AVAssetReader setup error:%@",[error localizedFailureReason]);
	}
...
//	[movieAsset release];
	...
	// add the avMovieReader => avVideoReaderOutput hook
	{
		...
		avVideoReaderOutput = [[AVAssetReaderTrackOutput assetReaderTrackOutputWithTrack:clipVideoTrack outputSettings:videoReaderSettings] retain];
		if([avMovieReader canAddOutput:avVideoReaderOutput]) {
			[avMovieReader addOutput:avVideoReaderOutput];
		}
	}
...
+(BOOL)checkMovieSession:(NSURL*)movieURL
{
	...
	NSError* error=nil;	
	AVAssetReader* movieReader = [[AVAssetReader assetReaderWithAsset:movieAsset error:&error] retain];
	AVAssetTrack *clipVideoTrack = [[movieAsset tracksWithMediaType:AVMediaTypeVideo] objectAtIndex:0];
	
	movieReader.timeRange = clipVideoTrack.timeRange;

		NSDictionary *videoOutputSettings = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:kCVPixelFormatType_32BGRA], kCVPixelBufferPixelFormatTypeKey,nil];		
		AVAssetReaderTrackOutput* videoReaderOutput  = [[AVAssetReaderTrackOutput assetReaderTrackOutputWithTrack:clipVideoTrack outputSettings:videoOutputSettings] retain];
	...
	CVImageBufferRef videoImageBufferRef = CMSampleBufferGetImageBuffer(videoSampleBufferRef);
	{
		...
		NSLog(@"Buffer Size(%f,%f) BytesPerRow:%d",bufferSize.width,bufferSize.height,bytesPerRow);
...
//#if 0
        if([[readMovieAsset tracksWithMediaType:AVMediaTypeAudio] count]>0)
		{
			...
			NSLog(@"Clip audio:CMTime(%d,%d)---CMTimeDuration(%d,%d)",audioClipStartTime.value,audioClipStartTime.timescale,clipTimeRange.duration.value,clipTimeRange.duration.timescale);
		}
//#endif
...
//	@try{
		...
		subMovieStarts[subMovieIndex] = lastSampleTimeRange.start;
		NSLog(@"Stop w/o audio %d at（%d,%d） video frame!",subMovieIndex,subMovieStarts[subMovieIndex].value,subMovieStarts[subMovieIndex].timescale);
...
-(void)startRenderMovie//:(BOOL)withoutAudio
{
	...
	if(tempWriteMoviePath)
		[tempWriteMoviePath release];
	tempWriteMoviePath = [[Utilities documentsPath:[NSString stringWithFormat:@"Sub%d_%@",subMovieIndex,@"bullet_movie.mov"]] retain];
	NSFileManager *manager = [NSFileManager defaultManager];
...
//		[self cancelMovieSession];
	...
	[tempWriteMoviePath release];
	tempWriteMoviePath = nil;
	[prevWriteMoviePath release]; //bret movielooks update
	prevWriteMoviePath = nil;
}
...
-(void)resumeRenderMovie//:(BOOL)withoutAudio
{
	...
	{	
		NSString* lastWriteMoviePath = [Utilities documentsPath:[NSString stringWithFormat:@"Sub%d_%@",subMovieIndex,@"bullet_movie.mov"]];
    ...
    if (subMovieIndex > 1 )
    {
        if(prevWriteMoviePath)
            [prevWriteMoviePath release];
        prevWriteMoviePath = [[Utilities documentsPath:[NSString stringWithFormat:@"Sub%d_%@",subMovieIndex-1,@"bullet_movie.mov"]] retain];
    }
...
//		[self stopMovieSession];
    ...
    //	if(subMovieIndex>1)
		[self.delegate performSelectorOnMainThread:@selector(finishRenderMovieEvent) withObject:nil waitUntilDone:NO];
	//else
	if(NO)
	{	
		movieRenderState = MovieStateComplete;

		[self.delegate performSelectorOnMainThread:@selector(finishProcessMovieEvent:) withObject:[Utilities documentsPath:[NSString stringWithFormat:@"Sub%d_%@",subMovieIndex,@"bullet_movie.mov"]] waitUntilDone:NO];
	}
}
...
-(void)startComposeMovie//:(BOOL)withoutAudio
{
	...
	tempWriteMoviePath = [[Utilities documentsPath:@"movie_looks.mov"] retain];
